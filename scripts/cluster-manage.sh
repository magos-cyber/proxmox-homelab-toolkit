...
# Logging functions
log() { echo -e "[$(date '+%Y-%m-%d %H:%M:%S')] ${BLUE}$1${NC}"; }
error() { echo -e "[$(date '+%Y-%m-%d %H:%M:%S')] ${RED}$1${NC}"; exit 1; }
warn() { echo -e "[$(date '+%Y-%m-%d %H:%M:%S')] ${YELLOW}$1${NC}"; }
success() { echo -e "[$(date '+%Y-%m-%d %H:%M:%S')] ${GREEN}$1${NC}"; }
